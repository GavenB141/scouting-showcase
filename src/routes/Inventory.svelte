...
<div class="box">
    ...
    <div>
        {#each ['cube','cone'] as piece}
            <button disabled={!state.started} class="bg-none border-none" on:click={()=>{logIntake(piece)}}>
...
<div class="h-16">
    <div class="flex flex-row w-fit flex-wrap h-10">
        {#each $inv as i (i.id)}
            <button disabled={!state.started} animate:flip in:fly out:slide on:click={()=>{logDrop(i.type,i.id)}} class="bg-none border-none">
                <img src={imgs[i.type]} alt={i.type} />
            </button>
        {/each}
    </div>
    {#if $inv.length > 0}
        <div transition:slide class="text-xs text-gray-400">Click piece if Dropped or Misplaced</div>
    {/if}
</div>