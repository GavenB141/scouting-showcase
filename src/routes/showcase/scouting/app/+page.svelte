...
<center class="pt-10" data-sveltekit-preload-data="off" data-sveltekit-reload>
    {#if step===0}
        <div in:fly={{duration:500, delay:500}} out:fly={{duration:500}}>
            <Pre host={data.host} bind:meta={meta} bind:pregame={pregame} events={data.events} on:advance={()=>{step++}}/>
        </div>
    {:else if step===1}
        <div in:fly={{duration:500, delay:500}} out:fly={{duration:500}}>
